# Section 04: React Essentials - Deep Dive

## This section focus on

▸ Behind the Secenes of JSX <br>
▸ Structuring Components and State <br>
▸ Advanced State Usage <br>
▸ Patterns and Best Practices <br>

- ### [01 React.createElement](./01-createElement.md)
- ### [02 Fragments](./02-fragments.md)
- ### [03 Forwarding Props](./03-forwarding-props.md)
- ### [04 JSX Slots](./04-jsx-slots.md)
- ### [05 Setting Component Types Dynamically](./05-setting-component-types-dynamically.md)
- ### [06 Setting Default Prop Values](./06-default-prop-values.md)